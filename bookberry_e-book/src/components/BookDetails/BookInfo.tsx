...
export const BookInfo: React.FC<Props> = ({ book }) => {
  const details = [
    { icon: <Calendar className="h-5 w-5" />, label: az.publishedYear, value: book.publishedYear },
    { icon: <BookIcon className="h-5 w-5" />, label: az.pages, value: book.pages },
    { icon: <Globe className="h-5 w-5" />, label: az.language, value: book.language },
    { icon: <Layout className="h-5 w-5" />, label: az.genre, value: book.genre }
  ].filter(detail => detail.value);

  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{az.bookDetails}</h2>
